...
static void freeproc(struct proc *p) {
    ...
    pagetable_t k_pt_1 = (pagetable_t)PTE2PA(p->k_pagetable[0]);
    for (int i = 0; i < 96; i++) {
        k_pt_1[i] = 0;
    }

    if (p->k_pagetable)
        freek_pgtbl(p->k_pagetable);
    p->k_pagetable = 0;
    if (p->pagetable)
...
void userinit(void) {
    ...
    p->sz = PGSIZE;
    mapu2k(p->pagetable, p->k_pagetable);
...
int fork(void) {
    ...
    mapu2k(np->pagetable, np->k_pagetable);
    ...
    for (;;) {
        ...
        for (p = proc; p < &proc[NPROC]; p++) {
            ...
            if (p->state == RUNNABLE) {
                ...
                p->state = RUNNING;
                c->proc = p;
                w_satp(MAKE_SATP(p->k_pagetable));  // switch pagetable
                sfence_vma();
                swtch(&c->context, &p->context);
                kvminithart();  // switch pagetable back
